	...
	public function generate() {
		...
		while (true) {
			...
			if (!preg_match('/^([A-Za-z0-9_]+|\s)+$/', $name) || is_numeric($name[0])) {
				$this->out('');
				$this->err(__d('migrations', 'Migration name ({0}) is invalid. It must only contain alphanumeric characters and start with a letter.', $name));
			} elseif (strlen($name) > 255) {
				$this->out('');
				$this->err(__d('migrations', 'Migration name ({0}) is invalid. It cannot be longer than 255 characters.', $name));
			} else {
 ...
 */
	public function beforeAction(&$Migration, $type, $data) {
		if (isset($this->__messages[$type])) {
			$message = Text::insert($this->__messages[$type], $data);
			$this->out('      > ' . $message);
		}
	}
