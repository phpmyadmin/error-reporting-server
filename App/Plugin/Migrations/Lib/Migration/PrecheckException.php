	...
	public function checkDropTable($table) {
		if (!$this->tableExists($table)) {
			throw new MigrationException($this->migration,
				__d('migrations', 'Table "{0}" does not exists in database.', $this->migration->db->fullTableName($table, false, false))
			);
		}
		return true;
	}
 ...
 */
	public function checkCreateTable($table) {
		if ($this->tableExists($table)) {
			throw new MigrationException($this->migration,
				__d('migrations', 'Table "{0}" already exists in database.', $this->migration->db->fullTableName($table, false, false))
			);
		}
		return true;
	}
