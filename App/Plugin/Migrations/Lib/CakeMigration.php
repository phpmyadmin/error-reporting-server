 ...
 */
	public function __construct($options = array()) {
		...
		if (empty($options['precheck'])) {
			...
			/* TODO: App::uses($class, $plugin . 'Lib/Migration'); */
		...
		foreach ($this->migration[$this->direction] as $type => $info) {
			switch ($type) {
				...
				case 'rename_field':
					...
					break;
				default:
					$message = __d('migrations', 'Migration action type ({0}) is not one of valid actions type.', $type);
		...
		foreach ($tables as $table => $fields) {
			if ($this->_invokePrecheck('beforeAction', 'create_table', array('table' => $table))) {
				...
				try {
					...
					throw new MigrationException($this, __d('migrations', 'SQL Error: {0}', $exception->getMessage()));
				}
			}
		...
		foreach ($tables as $oldName => $newName) {
			if ($this->_invokePrecheck('beforeAction', 'rename_table', array('old_name' => $oldName, 'new_name' => $newName))) {
				...
				if (@$this->db->execute($sql) === false) {
					throw new MigrationException($this, __d('migrations', 'SQL Error: {0}', $this->db->error));
				}
